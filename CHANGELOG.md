...
- The beginning of Linux support ([ADR 0012](docs/decisions/0012-linux-support.md)): systemd
  ...
  plist and manager; a secret-reference router; and the two Linux secret backends
  (`systemd-creds:` for a headless host, `secret-tool:` for a desktop). Their command lines are
  written but **not yet verified on a Linux host**. It carries no `Environment=`,
